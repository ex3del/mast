...
2. **Дерево git должно быть чистым до применения правок.** Тогда весь результат ревизии
   виден через `git diff`, а откат — одна команда; это надёжнее любых резервных копий.
   Дерево грязное — ничего не применяй, попроси сначала закоммитить.
...
- **Версия Claude Code** — `claude --version`. Сравни с минимальной 2.1.234 по номерам
  major.minor.patch (ограждение 4). Ниже минимальной — зафиксируй это в описи; сам по себе
  этот факт не повод останавливаться, ни один файл на этом шаге ещё не пишется.
- **Чистота дерева git** — `git status --porcelain`. Не пусто — зафиксируй это в описи
  (ограждение 2): на сам шаг 1 и шаг 2 это не влияет, а вот шаг 3 без чистого дерева не
  начнётся.
...
Шаблоны `${CLAUDE_PLUGIN_ROOT}/locales/<язык>/templates/*.template.md` недоступны для
чтения (бывает — доступ сессии ограничен рабочим каталогом проекта) — не строй дифф
`ROADMAP.md` по памяти, по тексту ядра метода в контексте формат воспроизводится не
точно и линт на шаге 3 такой дифф не пройдёт. Скажи прямо, что шаблоны недоступны, и
на этом для мигрируемых файлов остановись — это тот же запрет на выдумывание, что и
в ограждении 1, только про формат, а не про число.
...
часть. Нет файла-кандидата ни на один пункт таблицы и все пять артефактов уже
на месте — сообщи, что каркас уже соответствует эталону, и останови сценарий без
вопросов.
...
Перед первой записью — ограждение 2: `git status --porcelain` ещё раз. Не пусто —
не пиши ничего, скажи человеку закоммитить сначала и покажи, что именно не
закоммичено; вернись к этому шагу после того, как дерево станет чистым.

Пусто — применяй только то, что приняли:

- недостающие артефакты — ровно то, что показано списком в шаге 2 (копии шаблонов,
  пустой `docs/roadmap/`, строка `.gitignore`);
- каждый принятый дифф по находкам — ровно как показан на шаге 2, ничего сверху не
  досочиняй (ограждение 1 остаётся в силе и здесь: если критерий пришёл без числа,
  он и после применения без числа);
