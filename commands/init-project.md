...
`TODO.md`, а всех пяти базовых артефактов разом: не сочиняй по памяти ни один из них,
ни `CLAUDE.md`, ни `ROADMAP.md`, ни `rule.template.md` для `.claude/rules/example.md` — по
...
- **файла не было, создаёшь с нуля** — пять базовых артефактов, `ROADMAP.md`, собранный
  из `TODO.md`, если `ROADMAP.md` ещё не существовал, `docs/roadmap/DONE.md` из
  `CHANGELOG.md`. Дерево на шаге 1 не проверяешь — нечего перезаписывать, откат для
  такого файла — `rm`. Было грязным — создавай всё равно, но предупреди человека одной
  строкой: эти файлы лягут поверх незакоммиченных изменений;
