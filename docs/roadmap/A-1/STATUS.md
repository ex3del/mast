...
- [x] **Шаг 2: Написать README на двух языках**
...
- [x] **Шаг 3: Проверить установку как посторонний**
...
- [x] **Шаг 4: Поднять версию и закоммитить**
...
- [x] **Шаг 3: Научить линт обоим языкам**
...
- [x] **Шаг 5: Прогнать весь набор и закоммитить**
...
## Task 10: Правило Context7 в поставке плагина

**Files:**
- Create: `locales/{ru,en}/templates/context7.rule.template.md`
- Modify: `commands/init-project.md`, `README.md`, `README.ru.md`

Спека обещает, что плагин несёт не только MCP-сервер Context7, но и правило «смотреть актуальную
документацию вместо памяти». Сервер подключён в Task 8, правила нет нигде — значит инструмент есть,
а указания им пользоваться нет.

Правило живёт шаблоном и попадает в проект пользователя через `/init-project`, как остальной каркас:
плагин не может и не должен писать в чужие файлы сам.

- [x] **Шаг 1: Написать падающий тест**

```python
# дописать в tests/test_templates.py
...
Запуск: `python3 -m pytest tests/test_templates.py -k context7 -v` → FAIL.

- [x] **Шаг 3: Написать шаблон правила в двух локалях**

Фронтматтер `paths:` с путями исходников проекта. Тело: смотреть документацию библиотек через
инструмент поиска документации вместо памяти; **имя сервера не называть** — у человека может быть
свой Context7, и правило должно работать с любым. Держать в пределах 15 строк.

- [x] **Шаг 4: Дописать команду и README**

В `commands/init-project.md` — правило предлагается как отдельный артефакт каркаса, отдельным вопросом;
если в проекте уже настроен свой сервер документации, свой не навязывается. В обоих README — строка
о том, что в комплекте идёт и сервер, и правило.

- [x] **Шаг 5: Прогнать весь набор и закоммитить**

```bash
python3 -m pytest -v
git add locales tests commands README.md README.ru.md
git commit -m "[A-1] правило Context7 в поставке, предлагается командой как часть каркаса"
```
...
- **`roadmap_lint.py` не понимал английские статусы/поля** (`planned`, `Done when`, …) — только русские (`запланирован`, «готово когда» и т.д.). Закрыто задачей 9: линт понимает оба языка (статусы, «Готово когда»/`Done when`, «Мои пути»/`My paths`, «Зависит от»/`Depends on`), оба шаблона `ROADMAP.template.md` проходят собственный линт.
- **Пятый артефакт каркаса живьём не создавался.** `/init-project` развернул вживую 4 файла из 5; `.claude/rules/example.md` не создан ни разу — запись в `.claude/` упирается в гейт доверия площадки к каталогу, а не в логику команды. Закрыто статическим тестом содержимого шаблона. Первое, что стоит проверить руками после установки у реального пользователя.
- **Применение правок к существующим файлам живьём не воспроизводилось.** Все прогоны были `--check` либо создание недостающих файлов; шаг «показать дифф и переписать чужой `CLAUDE.md`» проверен только по тексту сценария, та же причина — гейт доверия. Второе, что стоит проверить руками.
- **Скиллы метода лежат в двух местах** — `~/Documents/claude/skills/` и `locales/*/skills/` плагина — и синхронизируются руками. Нужен единый источник: либо репозиторий `claude` становится upstream плагина, либо наоборот. До решения правка скилла обязана идти в обе копии одним заходом.
