...
- **Линт молчит на чужом формате.** Хук `PostToolUse` срабатывает на любой `ROADMAP.md`. Ноль распознанных строк пунктов при непустом файле — выходим без единого слова, иначе плагин ругается при каждом сохранении чужого файла.
