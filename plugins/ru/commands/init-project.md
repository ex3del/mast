...
3. **Ни один файл не перезаписывается молча.** Есть файл — дифф «было → станет» и отдельный
   вопрос по нему. Дифф показывается блоком `diff` со строками `-` и `+`, а не текстом файла
   целиком: человек должен видеть, что меняется, а не перечитывать файл. Отказ человека —
   законный исход, а не ошибка, которую надо обойти.
...
|---|---|
| `TODO.md` или самодельный роадмап | пункты в наш формат: раздел буквой, номер, статус, «Мои пути», «Готово когда» — дифф показывает предлагаемый текст `ROADMAP.md`. Критерий без числа в оригинале — переносится текстом и помечается незаданным (ограждение 1), не сочиняется. И отдельный вопрос: удалить ли оригинал после переноса? Иначе в проекте две истины об одном |
...
| `CHANGELOG.md` с закрытой работой | тезисы в `docs/roadmap/DONE.md`, по две строки на пункт — дифф добавляемых строк. И отдельный вопрос: удалить ли оригинал после переноса? |
...
или отдельной строкой сразу под ним — выше «Мои пути» и «Готово когда», а не внутрь
критерия и не под ним: линт читает критерий вместе со строками переноса до следующего
поля, и цифры, которые описывают проблему, а не замер, он примет за заданный критерий —
ограждение 1 перестанет быть видно линту на шаге 3.
...
- **файл уже существовал, меняешь его на месте** — длинный `CLAUDE.md`, правило без
  ...
  удаление перенесённого оригинала (`git rm`, и только после того, как копия записана).
