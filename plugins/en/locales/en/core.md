...
- Before `superpowers:writing-plans` — always the skill `mast:managing-roadmap-items`: the item is created in `ROADMAP.md` first, then the item's session creates `STATUS.md` and fills the plan. `brainstorming` calls `writing-plans` directly — this order overrides that.
