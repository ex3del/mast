...
class LintTest(unittest.TestCase):
    ...
    def test_число_на_строке_переноса_критерия_засчитывается(self):
        # Проекты переносят критерий по ширине строки: число уезжает на вторую строку
        text = OK.replace("Готово когда: пустой отчёт → 200.",
                          "Готово когда: пустой отчёт отдаётся без ошибки и без таймаута\n"
                          "    на холодном кэше, ответ за < 300 мс.")
        self.assertEqual(lint(text), [])

    def test_перенос_без_числа_остаётся_ошибкой(self):
        text = OK.replace("Готово когда: пустой отчёт → 200.",
                          "Готово когда: пустой отчёт отдаётся без ошибки\n    и без таймаута.")
        self.assertIn("B-2: в «Готово когда» нет числа", lint(text))

    def test_число_из_следующего_поля_не_засчитывается_критерию(self):
        text = OK.replace("Готово когда: пустой отчёт → 200.",
                          "Готово когда: экспорт работает.\n  Мои пути: api/v2/**")
        self.assertIn("B-2: в «Готово когда» нет числа", lint(text))

    def test_не_задан_остаётся_незаданным_даже_с_цифрами_ниже(self):
        # Перенос из TODO.md: формулировка с цифрами проблемы строкой ниже не делает
        # критерий заданным — иначе ограждение «критерий не выдумывается» ослепнет
        text = OK.replace("Готово когда: пустой отчёт → 200.",
                          "Готово когда: не задан\n  Было: накрутил 10000 задач, экспорт упал.")
        self.assertIn("B-2: в «Готово когда» нет числа", lint(text))
