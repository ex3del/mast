...
1. Start: `claude --bg --worktree A-1 --name A-1` plus `--model` per the rule in "Roadmap dispatcher" — a background session, its working copy in `.claude/worktrees/A-1/`, branch `worktree-A-1`, base `origin/main` (your repository's default branch; here and below, swap in your own if it's named differently, e.g. `master`). The command prints an id: attach with `claude attach <id>`, list with `claude agents`. Without `--bg` the session opens in the current terminal. The item's line in `ROADMAP.md` must be pushed before the start, or the worktree won't see it.
...
- An item's model defaults to the stronger one. Downgrading to the more economical one — only if the error is discoverable by checking against something existing, reversible with a single `git revert`, and hits exactly one item. Zones that always need the stronger model are declared by the project in `.claude/rules/dispatch.md`; no file — everything goes on the stronger model, a file exists (even with no zones) — the more economical one is allowed under these three conditions.
