...
| `docs/roadmap/DONE.md` | at the first closed item | closed items, 2 lines each: what changed with a number, date, commit range, links to `done/<item>/STATUS.md` and an ADR if they exist. Dropped items go in a "Dropped" subsection, one line each with the reason. The first layer for "has this been done already?" and "has this been tried and rejected?" |
